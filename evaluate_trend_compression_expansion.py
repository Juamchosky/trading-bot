...
from bot.strategy.trend_compression_expansion import (
...
OUTPUT_PATH = Path("trend_compression_expansion_evaluation.csv")


@dataclass(frozen=True)
class TrendCompressionExpansionConfig:
    symbol: str = "BTCUSDT"
...
@dataclass(frozen=True)
class TrendCompressionExpansionEvaluation:
    strategy_name: str
    symbol: str
    interval: str
    candle_count: int
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluacion offline de estrategia trend compression expansion."
    )
    parser.add_argument("--symbol", default=TrendCompressionExpansionConfig.symbol)
    parser.add_argument(
        "--interval",
        default=TrendCompressionExpansionConfig.binance_interval,
    )
    parser.add_argument(
        "--candle-count",
        type=int,
        default=TrendCompressionExpansionConfig.candle_count,
    )
    parser.add_argument(
        "--historical-offset",
        type=int,
        default=TrendCompressionExpansionConfig.historical_offset,
    )
    parser.add_argument(
        "--initial-cash",
        type=float,
        default=TrendCompressionExpansionConfig.initial_cash,
    )
    parser.add_argument(
        "--fee-rate",
        type=float,
        default=TrendCompressionExpansionConfig.fee_rate,
    )
    parser.add_argument(
        "--position-size-pct",
        type=float,
        default=TrendCompressionExpansionConfig.position_size_pct,
    )
    parser.add_argument("--output-path", type=Path, default=OUTPUT_PATH)
    return parser.parse_args()


def load_candles(config: TrendCompressionExpansionConfig) -> list[Candle]:
    try:
...
def evaluate_strategy(
    config: TrendCompressionExpansionConfig,
    candles: Sequence[Candle],
) -> TrendCompressionExpansionEvaluation:
    strategy = TrendCompressionExpansionStrategy()
    broker = PaperBroker(
    ...
    for candle in candles:
        ...
        if (
            ...
            and candle.low <= current_stop_price
        ):
            ...
            stop_candidate = strategy.initial_stop_price()
            if stop_candidate < candle.close:
                trade = broker.buy_all(candle.close)
                if trade is not None:
                    total_buys_executed += 1
                    current_stop_price = stop_candidate
    ...
    return TrendCompressionExpansionEvaluation(
        strategy_name="trend_compression_expansion_sma50_slope10_range1p2_body0p5_tp2r",
        symbol=config.symbol,
        interval=config.binance_interval,
...
def export_evaluation_csv(
    row: TrendCompressionExpansionEvaluation,
    output_path: Path,
) -> None:
...
def print_evaluation(
    ...
    print("Evaluacion trend compression expansion")
