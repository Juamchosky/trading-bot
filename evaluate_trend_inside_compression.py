...
OUTPUT_PATH = Path("trend_inside_compression_evaluation.csv")


@dataclass(frozen=True)
class TrendInsideCompressionConfig:
    symbol: str = "BTCUSDT"
...
@dataclass(frozen=True)
class TrendInsideCompressionEvaluation:
    strategy_name: str
    symbol: str
    interval: str
    candle_count: int
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluacion offline de estrategia trend inside compression minimalista."
    )
    parser.add_argument("--symbol", default=TrendInsideCompressionConfig.symbol)
    parser.add_argument("--interval", default=TrendInsideCompressionConfig.binance_interval)
    parser.add_argument(
        "--candle-count",
        type=int,
        default=TrendInsideCompressionConfig.candle_count,
    )
    parser.add_argument(
        "--historical-offset",
        type=int,
        default=TrendInsideCompressionConfig.historical_offset,
    )
    parser.add_argument(
        "--initial-cash",
        type=float,
        default=TrendInsideCompressionConfig.initial_cash,
    )
    parser.add_argument("--fee-rate", type=float, default=TrendInsideCompressionConfig.fee_rate)
    parser.add_argument(
        "--position-size-pct",
        type=float,
        default=TrendInsideCompressionConfig.position_size_pct,
    )
    parser.add_argument("--output-path", type=Path, default=OUTPUT_PATH)
    return parser.parse_args()


def load_candles(config: TrendInsideCompressionConfig) -> list[Candle]:
    try:
...
def evaluate_strategy(
    config: TrendInsideCompressionConfig,
    candles: Sequence[Candle],
) -> TrendInsideCompressionEvaluation:
    strategy = TrendInsideCompressionStrategy()
    broker = PaperBroker(
    ...
    return TrendInsideCompressionEvaluation(
        strategy_name="trend_inside_compression_sma50_slope10_tp2r",
        symbol=config.symbol,
        interval=config.binance_interval,
...
def export_evaluation_csv(
    row: TrendInsideCompressionEvaluation,
    output_path: Path,
) -> None:
...
def print_evaluation(row: TrendInsideCompressionEvaluation, output_path: Path) -> None:
    print("Evaluacion trend inside compression")
