...
OUTPUT_PATH = Path("trend_breakout_evaluation.csv")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluacion offline de estrategia trend breakout minimalista."
    )
    parser.add_argument("--symbol", default=TrendBreakoutConfig.symbol)
    parser.add_argument("--interval", default=TrendBreakoutConfig.binance_interval)
    parser.add_argument("--candle-count", type=int, default=TrendBreakoutConfig.candle_count)
    parser.add_argument(
        "--historical-offset",
        type=int,
        default=TrendBreakoutConfig.historical_offset,
    )
    parser.add_argument("--initial-cash", type=float, default=TrendBreakoutConfig.initial_cash)
    parser.add_argument("--fee-rate", type=float, default=TrendBreakoutConfig.fee_rate)
    parser.add_argument(
        "--position-size-pct",
        type=float,
        default=TrendBreakoutConfig.position_size_pct,
    )
    parser.add_argument("--output-path", type=Path, default=OUTPUT_PATH)
    return parser.parse_args()


def load_candles(config: TrendBreakoutConfig) -> list[Candle]:
    try:
...
def evaluate_strategy(
    ...
    return TrendBreakoutEvaluation(
        strategy_name="trend_breakout_sma50_slope10_high10_tp2r",
        symbol=config.symbol,
        interval=config.binance_interval,
...
def export_evaluation_csv(row: TrendBreakoutEvaluation, output_path: Path) -> None:
...
def print_evaluation(row: TrendBreakoutEvaluation, output_path: Path) -> None:
    print("Evaluacion trend breakout")
