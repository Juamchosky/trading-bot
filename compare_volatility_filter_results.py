...
GROUP_OFF = "volatility filter off"
GROUP_ON = "volatility filter on"
TREND_FILTER_FIXED = True
...
def main() -> None:
    ...
    skipped_invalid_trend = 0
    skipped_non_fixed_trend = 0
    skipped_invalid_volatility = 0

    for row in rows:
        trend_enabled = parse_bool(row.get("trend_filter_enabled"))
        if trend_enabled is None:
            skipped_invalid_trend += 1
            continue
        if trend_enabled != TREND_FILTER_FIXED:
            skipped_non_fixed_trend += 1
            continue

        volatility_enabled = parse_bool(row.get("volatility_filter_enabled"))
        if volatility_enabled is None:
            skipped_invalid_volatility += 1
            continue

        group_key = GROUP_ON if volatility_enabled else GROUP_OFF
        grouped_rows[group_key].append(row)

    print(f"Archivo analizado: {SUMMARY_PATH}")
    print(f"Total corridas leidas: {len(rows)}")
    print(f"Trend filter fijo en: {TREND_FILTER_FIXED}")
    if skipped_invalid_trend:
        print(
            "Corridas omitidas por trend_filter_enabled invalido/vacio: "
            f"{skipped_invalid_trend}"
        )
    if skipped_non_fixed_trend:
        print(
            "Corridas omitidas por trend_filter_enabled distinto al valor fijo: "
            f"{skipped_non_fixed_trend}"
        )
    if skipped_invalid_volatility:
        print(
            "Corridas omitidas por volatility_filter_enabled invalido/vacio: "
            f"{skipped_invalid_volatility}"
        )

    print_group("Grupo: volatility filter off", summarize_group(grouped_rows[GROUP_OFF]))
    print_group("Grupo: volatility filter on", summarize_group(grouped_rows[GROUP_ON]))
