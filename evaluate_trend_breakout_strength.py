...
OUTPUT_PATH = Path("trend_breakout_strength_evaluation.csv")


@dataclass(frozen=True)
class TrendBreakoutStrengthConfig:
    symbol: str = "BTCUSDT"
...
@dataclass(frozen=True)
class TrendBreakoutStrengthEvaluation:
    strategy_name: str
    symbol: str
    interval: str
    candle_count: int
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluacion offline de estrategia trend breakout strength minimalista."
    )
    parser.add_argument("--symbol", default=TrendBreakoutStrengthConfig.symbol)
    parser.add_argument(
        "--interval",
        default=TrendBreakoutStrengthConfig.binance_interval,
    )
    parser.add_argument(
        "--candle-count",
        type=int,
        default=TrendBreakoutStrengthConfig.candle_count,
    )
    parser.add_argument(
        "--historical-offset",
        type=int,
        default=TrendBreakoutStrengthConfig.historical_offset,
    )
    parser.add_argument(
        "--initial-cash",
        type=float,
        default=TrendBreakoutStrengthConfig.initial_cash,
    )
    parser.add_argument("--fee-rate", type=float, default=TrendBreakoutStrengthConfig.fee_rate)
    parser.add_argument(
        "--position-size-pct",
        type=float,
        default=TrendBreakoutStrengthConfig.position_size_pct,
    )
    parser.add_argument("--output-path", type=Path, default=OUTPUT_PATH)
    return parser.parse_args()


def load_candles(config: TrendBreakoutStrengthConfig) -> list[Candle]:
    try:
...
def evaluate_strategy(
    config: TrendBreakoutStrengthConfig,
    candles: Sequence[Candle],
) -> TrendBreakoutStrengthEvaluation:
    strategy = TrendBreakoutStrengthStrategy()
    broker = PaperBroker(
    ...
    return TrendBreakoutStrengthEvaluation(
        strategy_name="trend_breakout_strength_sma50_slope10_high10_body05_tp2r",
        symbol=config.symbol,
        interval=config.binance_interval,
...
def export_evaluation_csv(
    row: TrendBreakoutStrengthEvaluation,
    output_path: Path,
) -> None:
...
def print_evaluation(row: TrendBreakoutStrengthEvaluation, output_path: Path) -> None:
    print("Evaluacion trend breakout strength")
