...
GROUP_OFF = "trend slope filter off"
GROUP_ON = "trend slope filter on"
...
def main() -> None:
    ...
    for row in rows:
        ...
        slope_enabled = parse_bool(row.get("trend_slope_filter_enabled"))
        if slope_enabled is None:
            skipped_invalid_slope += 1
            continue

        group_key = GROUP_ON if slope_enabled else GROUP_OFF
    ...
    if skipped_invalid_slope:
        print(
            "Corridas omitidas por trend_slope_filter_enabled invalido/vacio: "
            f"{skipped_invalid_slope}"
        )

    print_group("Grupo: trend slope filter off", summarize_group(grouped_rows[GROUP_OFF]))
    print_group("Grupo: trend slope filter on", summarize_group(grouped_rows[GROUP_ON]))
