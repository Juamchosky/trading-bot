...
OUTPUT_PATH = Path("time_series_momentum_evaluation.csv")
...
@dataclass(frozen=True)
class TimeSeriesMomentumEvaluation:
    strategy_name: str
    symbol: str
    interval: str
    candle_count: int
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluacion offline de baseline time-series momentum single-horizon."
    )
    parser.add_argument("--symbol", default=TimeSeriesMomentumConfig.symbol)
    parser.add_argument("--interval", default=TimeSeriesMomentumConfig.binance_interval)
    parser.add_argument("--candle-count", type=int, default=TimeSeriesMomentumConfig.candle_count)
    parser.add_argument(
        "--historical-offset",
        type=int,
        default=TimeSeriesMomentumConfig.historical_offset,
    )
    parser.add_argument("--initial-cash", type=float, default=TimeSeriesMomentumConfig.initial_cash)
    parser.add_argument("--fee-rate", type=float, default=TimeSeriesMomentumConfig.fee_rate)
    parser.add_argument(
        "--position-size-pct",
        type=float,
        default=TimeSeriesMomentumConfig.position_size_pct,
    )
    parser.add_argument(
        "--lookback-period",
        type=int,
        default=TimeSeriesMomentumConfig.lookback_period,
    )
    parser.add_argument("--output-path", type=Path, default=OUTPUT_PATH)
    return parser.parse_args()


def load_candles(config: TimeSeriesMomentumConfig) -> list[Candle]:
    try:
...
def evaluate_strategy(
...
) -> TimeSeriesMomentumEvaluation:
    strategy = TimeSeriesMomentumStrategy(lookback_period=config.lookback_period)
    broker = PaperBroker(
    ...
    return TimeSeriesMomentumEvaluation(
        strategy_name=f"time_series_momentum_l{config.lookback_period}_long_flat",
        symbol=config.symbol,
        interval=config.binance_interval,
...
def export_evaluation_csv(
    row: TimeSeriesMomentumEvaluation,
    output_path: Path,
) -> None:
...
def print_evaluation(row: TimeSeriesMomentumEvaluation, output_path: Path) -> None:
    print("Evaluacion time-series momentum single-horizon")
    ...
    print(f"lookback_period: {row.lookback_period}")
    print(
